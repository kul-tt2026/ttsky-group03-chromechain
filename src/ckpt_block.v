// ckpt_block -- the Fase-1 integration: 13 verified modules, one datapath.
//
// Until this file existed, no module in this repo instantiated another except the three
// exit trees sharing max2_node (docs/cc_rtl_completion_roadmap.md "Waar we staan").
// checkpoint_ctrl emitted capture / rd_grp / rom_addr / sgn / bias_sh / oacc_* / t_sel
// into open air. That is why ALL EIGHT invariants of
// docs/cc_verilog_build_prompts.md §0.1 were untested: every one of them is a property
// of a WIRE, not of a module, and the wires did not exist. This module is those wires.
//
// ---------------------------------------------------------------- the netlist
//   bitplane_buffer.plane --+--> popcount.bits --> .count --> active_pixel_scan.pop
//                           +--> active_pixel_scan.plane
//   active_pixel_scan.pix_idx     --> w1_addr (PORT) --> w1_row --> l1_horner_acc.w_col
//   active_pixel_scan.pix_act     --> l1_horner_acc.act
//   active_pixel_scan.plane_start --> l1_horner_acc.plane_start        (the x2)
//   active_pixel_scan.plane_end   --> checkpoint_ctrl.plane_end
//   l1_horner_acc.acc_NEXT        --> l1_acc_shadow(_cg).acc_live      (see BOUNDARY SKEW)
//   checkpoint_ctrl.capture/rd_grp--> l1_acc_shadow(_cg)
//   l1_acc_shadow.a1_out          --> `P x requant_unit.a1
//   checkpoint_ctrl.rom_addr      --> requant_rom_x4 + l2_weight_rom_x4 (shared bus, I8)
//   checkpoint_ctrl.sgn           --> requant_unit.sgn                 (I1: tied 0)
//   checkpoint_ctrl.bias_sh       --> the arithmetic shifter on bias    (I2)
//   `P x requant_unit.h           --> l2_mac_x4.h_in
//   l2_mac_x4.acc_out --> [out-ACC, `NCLASS x `OACC_W] --> l2_mac_x4.acc_in  (feedback)
//   checkpoint_ctrl.oacc_init/_sel_init/_en --> that register            (I3: b2)
//   out-ACC --> exit_tree_2stage.y ; checkpoint_ctrl.t_sel --> .T        (I4)
//   exit_tree_2stage.done/argmax --> checkpoint_ctrl.tree_done/tree_argmax
//   config_latch.t_cfg/ckpt_en/en_skip/inv_plane/en_vstrobe --> the rest
//
// ---------------------------------------------------------------- THE NEW LOGIC
// The out-ACC register, `NCLASS x `OACC_W = 120 flops, is the only logic this file
// adds. l2_mac_x4 is purely combinational (acc_in -> acc_out), so the accumulator it
// feeds back into exists nowhere else; checkpoint_ctrl already carries oacc_init,
// oacc_sel_init and oacc_en for it. Both storage codings are parameterised rather than
// asserted -- see OACC_CG below -- because the l1_acc_shadow vs l1_acc_shadow_cg A/B
// showed the 120-flop enable-vs-clock-gate choice is worth ~0.1 t and should be
// measured, not argued.
//
// ---------------------------------------------------------------- BOUNDARY SKEW
// The one place this file deliberately departs from the roadmap's netlist drawing, and
// the reason to read the port lists rather than the diagram.
//
// The diagram says `l1_horner_acc.acc_live --> l1_acc_shadow_cg.acc_live`. That is a
// silent wrong answer. `plane_end` is asserted DURING a plane's last accumulate cycle
// (active_pixel_scan.v's cycle contract); checkpoint_ctrl derives `capture`
// combinationally from it; the shadow therefore latches at the posedge that ENDS that
// cycle -- the same edge that clocks the plane's last pixel into the accumulator. The
// flop OUTPUT at that edge is the ACC after len-1 pixels, one pixel short of the plane
// boundary. Measured, not deduced: with a plane of popcount 8 and a +1 weight, acc_live
// reads 7 on the plane_end cycle and 8 one cycle later.
//
// Moving `capture` a cycle later does not fix it -- the shadow's first read is at S+1
// (checkpoint_ctrl.v's cycle map) and shifting the check breaks
// dec == end_of_plane(k) + `GAMMA, i.e. cycle-exactness against engine.py's
// boundary_cycles(overlap=True). So the snapshot is taken from the accumulator's D side
// (l1_horner_acc.acc_next, added for this and costing no cell): snap gets exactly what
// q gets at that edge, which IS the plane-boundary ACC.
//
// AND IT WOULD HAVE SHIPPED, because the two orderings disagree about whether the last
// scanned cycle carries a pixel at all. Dense always ends on pixel 63, the 8x8 corner,
// which is nonzero in 1 image of the 10k test set -- so mode 1, the fixed-precision
// control arm, is clean. Zero-skip ends on a real pixel whenever popcount >= `ZS_FILL:
// 27,791 of 40,000 planes. The wrong wire is silent in the arm that would be trusted and
// wrong in 69.5% of the planes of the arm the headline number comes from. The mutation
// acc_next -> acc_live SURVIVES tb_ckpt_block's dense phase and dies on image 0 of its
// zero-skip phase.
//
// The deferred final snapshot is unaffected. After the last plane the scanner is idle,
// so act = plane_start = 0 and acc_next == acc_live == the frozen final ACC, whichever
// cycle the final check actually starts on.
//
// ---------------------------------------------------------------- W1 ROM: A PORT
// `w1_addr` out, `w1_row` in, no instance -- inheriting l1_horner_acc.v's own reasoning
// verbatim, and for one more reason that is specific to this block:
//   1. it is already priced on its own in w1_rom_synthesis/results.md, so instantiating
//      it here would double-count it against the option-C budget;
//   2. one ROM row is shared by all `NHID lanes, so there is exactly one address per
//      cycle and nothing to fan out;
//   3. DECISIVE: w1_h32_case.v's checked-in content is a DIFFERENT fold --
//      ternary_tapeout/artifacts/H32_fold_a_shift.npz (w1_rom_synthesis/results.md:7),
//      not the FINAL4_s12_fold_a.npz that engine.py, requant_rom_x4 and
//      l2_weight_rom_x4 all use. Instantiating it would make the Fase-1 gate --
//      bit-exact against engine.py's true_score_at() -- unreachable by construction,
//      because the L1 half of the datapath would be computing a different network from
//      the L2 half. The same call dump_l1_vectors.py already makes for build-l1.
// So ckpt_block's measured area EXCLUDES the W1 ROM. That is the honest split, not an
// omission: add w1_rom_synthesis/results.md's own w1_h32_case figure to compare against
// any whole-chip total.
//
// ---------------------------------------------------------------- NOT IN SCOPE
// No FSM. `img_start`, `swap` and `scan_start` are inputs, because the load | L1 |
// check | resume sequencer, the N_cap compare and the plane/pixel counters are Fase 3
// (`cc_top.v` + `top_fsm.v`). The block is written so the SIMPLEST correct sequencer is
// sufficient and contiguous: assert `swap` during a plane's `plane_end` cycle and
// `scan_start` on the next cycle, and planes cost exactly their own lengths with no
// per-plane bubble (active_pixel_scan.v's no-bubble contract). ckpt_tb/tb_ckpt_block.v
// does exactly that and checks the resulting decision cycles against engine.py.
//
// K10 SCOPE, restated because this file is where it would bite: `inv_plane` is wired
// through to bitplane_buffer and works, but inverting the bits fed to the MAC changes
// the arithmetic (sum W1*(1-b) = rowsum - sum W1*b) and the compensating row-sum offset
// is stage 3's, which does not exist. Page 1 is `CFG_INV_DEFAULT = 0 and every number
// in this repo is measured there; tb_ckpt_block asserts inv_plane stays 0. Page 2 must
// not tape out on K10 until the L1 correction exists (bitplane_buffer.v's own header).
//
