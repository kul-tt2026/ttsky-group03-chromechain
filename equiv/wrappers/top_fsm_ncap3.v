// Wrapper for formal_wrap.sh: top_fsm with n_cap tied to 3, to prove P3b changes
// nothing for this n_cap value (or, for 1..3, to show exactly where it differs).
`include "ckpt_defs.vh"
module top_fsm_ncap3 (
    ...
    top_fsm u (.clk(clk), .rst(rst), .start(start), .blob_loaded(blob_loaded),
        .n_cap(3'd3), .fill_full(fill_full), .scan_busy(scan_busy), .plane_end(plane_end),
